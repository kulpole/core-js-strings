...
function removeFirstOccurrences(str, value) {
  ...
  return (
    ...
    str.substr(str.indexOf(value) + value.length)
  );
}
...
function removeLastOccurrences(str, value) {
  ...
  return (
    str.substr(0, str.lastIndexOf(value)) +
    str.substr(str.lastIndexOf(value) + value.length)
  );
}
...
function formatTime(minutes, seconds) {
  ...
  if (seconds < 10) {
    ...
    result += String(seconds).padStart(1, '0');
  }
  return result;
}
...
function isPalindrome(str) {
  const foramatStr = str.toLowerCase().replace(/[^a-zA-Z]+/g, '');
  if (foramatStr === foramatStr.split('').reverse().join('')) {
    return true;
  }
  return false;
}
...
function encodeToRot13(str) {
  const code1 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz ?!';
  const code2 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm ?!';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    result += code2[code1.indexOf(str[i])];
  }
  return result;
}
